...
class RNN:
	...
	if node.isLeaf:
		...
		cost = retl[0] + retr[0]
		total = retl[1] + retr[1]
		node.hActs1 = np.dot(self.W, np.concatenate((node.left.hActs1, node.right.hActs1))) + self.b
		node.hActs1[node.hActs1 <0] = 0
		node.probs = np.dot(self.Ws, node.hActs1) + self.bs
		node.probs = np.exp(node.probs - np.max(node.probs))
		node.probs = node.probs/np.sum(node.probs)
	...
	if node.isLeaf:
		...
		self.db += del1
		self.dW += np.outer(del1, np.concatenate((node.left.hActs1, node.right.hActs1)))
		delbelow = np.dot(np.transpose(self.W), del1)
